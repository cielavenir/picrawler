...
class Picrawler::Fg
	...
	def member_first(options={})
		setup(options)
		ret=member_next
		if ret then @notifier.call 'Browsing http://www.fg-site.net/contents/view/user_id:'+@arg+"/\n" end
		return ret
	end
	...
	def search_first(options={})
		setup(options)
		ret=search_next
		if ret then @notifier.call 'Browsing http://www.fg-site.net/contents/search/sort:created/direction:desc/word:'+@arg+"/\n" end
		return ret
	end
